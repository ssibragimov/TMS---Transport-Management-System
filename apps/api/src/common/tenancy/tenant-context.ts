...
export interface TenantContext {
  ...
  userId: number | null;
  /**
   * Права текущего пользователя из токена.
   *
   * Нужны сервисам, где одно право решает не «пускать ли в обработчик»
   * (это делает guard), а как выполнить операцию: например, разрешено ли
   * выдать путевой лист в обход медосмотра. Пусто у фоновых задач.
   */
